...
def health():
    return jsonify({"status": "ok", "message": "API funcionando na Vercel"})
...
def agenda_funcionario(funcionario_id):
    agenda_items = [item for item in AGENDA if item['funcionario'] == funcionario_id]
    return jsonify(agenda_items)
